...
# 상세 분류 보고서의 개수(0 -> 245, 1 -> 155)는 VANET 교통 환경에서 정상 주행이 비상 상황보다 빈번한 실제 데이터 분포를 그대로 반영한 결과입니다. 상세 분류 보고서의 0은 정상, 1은 위험(사고 또는 비상상황)
# 학습/테스트 데이터로 분할하는 핵심 이유는 모델이 데이터의 정답을 단순 암기하지 않고, 새로운 도로 상황에 대한 일반화 성능을 확보하기 위함입니다.
# 이렇게 확보된 균형 잡힌 테스트셋은 모델이 실전 환경과 동일한 조건에서 자신의 예측 성능을 객관적으로 검증받을 수 있는 기준이 됩니다.
# 결과적으로 이 개수는 단순한 샘플 245, 155개수가 아니라, 모델이 현실의 데이터 흐름을 얼마나 정확하고 신뢰성 있게 처리하는지를 보여주는 진단 지표라고 생각하면 됩니다.
